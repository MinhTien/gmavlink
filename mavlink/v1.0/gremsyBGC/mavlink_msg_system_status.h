...
static __inline void mavlink_msg_system_status_send(mavlink_channel_t chan, float battery_voltage, uint8_t imu_calib, uint8_t sat_numbers, uint8_t system_activated, uint8_t status2)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[8];
	_mav_put_float(buf, 0, battery_voltage);
 ...
 */
static __inline uint8_t mavlink_msg_system_status_get_system_activated(const mavlink_message_t* msg)
{
