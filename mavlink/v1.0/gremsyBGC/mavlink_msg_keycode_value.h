...
#define MAVLINK_MESSAGE_INFO_KEYCODE_VALUE { \
	...
	{  { "keycode", NULL, MAVLINK_TYPE_UINT32_T, 0, 0, offsetof(mavlink_keycode_value_t, keycode) }, \
         { "device_name", NULL, MAVLINK_TYPE_UINT8_T, 0, 4, offsetof(mavlink_keycode_value_t, device_name) }, \
         } \
}
						       ...
						       uint8_t device_name, uint32_t keycode)
{
	...
	_mav_put_uint8_t(buf, 4, device_name);
	...
	return mavlink_finalize_message(msg, system_id, component_id, 5, 64);
}
						           ...
						           uint8_t device_name,uint32_t keycode)
{
	...
	msg->msgid = MAVLINK_MSG_ID_KEYCODE_VALUE;
	return mavlink_finalize_message_chan(msg, system_id, component_id, chan, 5, 64);
}
...
static __inline void mavlink_msg_keycode_value_send(mavlink_channel_t chan, uint8_t device_name, uint32_t keycode)
{
	...
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_KEYCODE_VALUE, buf, 5, 64);
#else
	mavlink_keycode_value_t packet;
	packet.keycode = keycode;
	packet.device_name = device_name;

	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_KEYCODE_VALUE, (const char *)&packet, 5, 64);
#endif
}
...
/**
 * @brief Get field device_name from keycode_value message
 *
...
static __inline void mavlink_msg_keycode_value_decode(const mavlink_message_t* msg, mavlink_keycode_value_t* keycode_value)
{
	...
	keycode_value->device_name = mavlink_msg_keycode_value_get_device_name(msg);
#else
